...
import { createTransaction, editTransaction } from "@/lib/utils";
...
import { getEditTransaction } from "@/hooks/usetransaction";
...
function CreateTransaction() {
  ...
  const { id } = useParams();
  const { edtisLoading, edterror, edttransaction }= getEditTransaction(id)
  ...
  useEffect(() => {
    if (edttransaction) {
      form.reset({
        description: edttransaction?.description || '',
        amount: edttransaction?.amount || '',
        type: edttransaction?.type || '',
        category: edttransaction?.category || '',
        date: moment(edttransaction?.date).format('YYYY-MM-DD') || '',
      });
    }
  }, [edttransaction, form]);
  ...
  if (isLoading || edtisLoading) {
    return <Loading />
  };

  return (
    <div className="container">
      <h2 className="text-center my-2 font-bold text-lg">Edit Transaction</h2>
